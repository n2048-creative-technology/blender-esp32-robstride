...
def main():
    ap = argparse.ArgumentParser(description="Run RobStride calibration via ESP32 serial (bounded sine)")
    ...
    try:
        seq = 1
        frame = pack_command(seq, 7, motor_id=args.id, timestamp_us=0)
        ser.write(frame)
        print(f"Sent CALIBRATE (cmd=7) for motor ID {args.id} to {args.port}")
        time.sleep(0.1)
    finally:
        ser.close()
