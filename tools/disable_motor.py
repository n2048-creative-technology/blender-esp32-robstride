...
def main():
    ap = argparse.ArgumentParser(description="Disable/Stop a RobStride motor via ESP32 serial")
    ...
    ap.add_argument("--stop", action="store_true", help="Send STOP (cmd=3) instead of DISABLE (cmd=2)")
    args = ap.parse_args()

    ser = serial.Serial(args.port, args.baud, timeout=0, write_timeout=1)
    try:
        seq = 1
        cmd = 3 if args.stop else 2
        frame = pack_command(seq, cmd, motor_id=args.id, timestamp_us=0)
        ser.write(frame)
        time.sleep(0.02)
        print(f"Sent {'STOP' if args.stop else 'DISABLE'} for motor ID {args.id} to {args.port}")
    finally:
        ser.close()
